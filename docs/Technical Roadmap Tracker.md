...
- [x] Weapon - Bomb: Right now the bomb weapon is acting just like a spear, shooting down a straight line. However what we want is for the bomb to "land" where the player is aiming and then cause area damage around that landing spot. Upgraded to: 
...
- [x] Weapon - Gun: The gun weapon bullets need to spread out slightly more when firing. right now they all go in the exact same line. This can be deterministic. The number of bullets in the burst should also be upgradeable (see upgrades)
- [*] Multiplayer: Create server and joining is overly complicated and currently doesn't work. I create an offer code by selecting host on device. (wizard implimented, STUN enabled, better flow).
